...
import { getChildGifts, getChildren } from "@/lib/data/parent";
...
export default async function KidsPage() {
  ...
  const giftLists = await Promise.all(kids.map((k) => getChildGifts(k.id)));
  const gifts = Object.fromEntries(kids.map((k, i) => [k.id, giftLists[i]]));
  return (
    <>
      ...
      <Suspense fallback={<ManagerFallback rows={3} />}>
        <KidsManager kids={kids} family={family} gifts={gifts} />
      </Suspense>
    </>
  );
}
